...
public class Oracle extends DataSource {
	...
	public Table getTable(String tableName) throws SQLException {
		...
		try {
			 ...
			 * 
			 * List rows=new ArrayList(); Map item=null; while(rs.next()) {
			 * item=new HashMap(); for(int i=1;i<=size;i++) {
			 * item.put(rsmd.getColumnName(i), rs.getString(i)); }
			 * rows.add(item); } System.out.println(rows);
			 */
	...
	private void getColumns(ResultSet rs, Table t) throws SQLException {
		while (rs.next()) {
			...
			col.setAutoincrement(rs.getBoolean("IS_AUTOINCREMENT"));
